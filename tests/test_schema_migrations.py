...
DCA_EXACT_AMOUNT = ["amount_text TEXT"]
...
def test_fresh_database_becomes_exact_v3(tmp_path, monkeypatch):
    db_path = tmp_path / "fresh.sqlite3"
    run_init(db_path, monkeypatch)
    assert user_version(db_path) == app.CURRENT_SCHEMA_VERSION == 3
...
def test_current_unstamped_schema_is_stamp_only(tmp_path, monkeypatch):
    ...
    assert ddl == []
    assert any("PRAGMA USER_VERSION = 3" in sql.upper() for sql in statements)


def test_exact_stamped_v3_is_validation_only(tmp_path, monkeypatch):
...
def test_stamped_v3_schema_drift_fails_without_mutation(
...
def test_prod_like_physical_order_adds_only_exact_amount_column(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(db_path) as db:
        columns = {row[1] for row in db.execute("PRAGMA table_info(dca_plans)")}
        assert "amount_text" in columns
    assert not any("SENT_TRANSACTIONS_NEW" in sql.upper() for sql in statements)
    assert not any("DROP TABLE SENT_TRANSACTIONS" in sql.upper() for sql in statements)
    ddl = [sql.upper() for sql in statements if sql.lstrip().upper().startswith(
        ("CREATE", "ALTER", "DROP")
    )]
    assert ddl == ["ALTER TABLE DCA_PLANS ADD COLUMN AMOUNT_TEXT TEXT"]
